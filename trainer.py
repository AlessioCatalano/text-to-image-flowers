...
def latest_checkpoint(checkpoint_dir, model_type):
    checkpoints = [ckpt for ckpt in os.listdir(checkpoint_dir) if ckpt.startswith(model_type)]
    print(checkpoints)
    if checkpoints:
        return max(checkpoints, key=lambda ckpt: int(ckpt.split('_')[1].replace('.pth','')))
    return None

class Trainer(object):
    def __init__(self, type, dataset, split, lr, diter, iter_log_file, epoch_log_file, save_path, l1_coef, l2_coef, pre_trained_gen, pre_trained_disc, batch_size, num_workers, epochs):
        ...
        if dataset == 'flowers':
            self.dataset = Text2ImageDataset('/content/drive/MyDrive/Colab Notebooks/Flowers/flowers.hdf5', split=split)
        else:
        ...
        self.logger = Logger(iter_log_file, epoch_log_file)
    ...
    def _train_wgan(self, cls):
        ...
        for epoch in range(self.num_epochs):
            ...
            while iterator < len(self.data_loader):
                ...
                self.logger.save_img(right_images, fake_images, epoch)
                self.logger.log_iteration_wgan(epoch, gen_iteration, d_loss, g_loss, real_loss, fake_loss)
                
            self.logger.log_epoch(gen_iteration)
    ...
    def _train_gan(self, cls):
        ...
        last_gen_checkpoint = latest_checkpoint(checkpoints_path, 'gen')
        last_disc_checkpoint = latest_checkpoint(checkpoints_path, 'disc')

        start_epoch = 0

        if last_gen_checkpoint and last_disc_checkpoint:
            gen_checkpoint_path = os.path.join(checkpoints_path, last_gen_checkpoint)
            disc_checkpoint_path = os.path.join(checkpoints_path, last_disc_checkpoint)

            gen_checkpoint = torch.load(gen_checkpoint_path)
            disc_checkpoint = torch.load(disc_checkpoint_path)

            print(int(last_gen_checkpoint.split('_')[-1].replace('.pth','')) + 1)

            self.generator.load_state_dict(gen_checkpoint)
            self.discriminator.load_state_dict(disc_checkpoint)

            # Assumendo che l'epoca sia salvata in uno dei checkpoint
            start_epoch = int(last_gen_checkpoint.split('_')[-1].replace('.pth','')) + 1
            print(f"Riprendendo l'addestramento da epoca {start_epoch}")
        else:
            print("Nessun checkpoint trovato, iniziando dall'epoca 0")
        ...
        for epoch in range(start_epoch, self.num_epochs):
            ...
            self.logger.log_epoch_w_scores(epoch)
    ...
    def _train_vanilla_gan(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            self.logger.log_epoch_w_scores(iteration)
    ...
    def predict(self):
        ...
        right_images = sample['right_images']
        right_embed = sample['right_embed']
        txt = 'insert your description about flowers'
        #print('right_image: ', right_images)
        print('sample[txt]: ', sample['txt'])
        ...
        #self.logger.save_img(right_images, fake_images, epoch=-1)

        #for image, t in zip(fake_images, txt):
            #print('image: ', image)
        im = Image.fromarray(fake_images[0].data.mul_(127.5).add_(127.5).byte().permute(1, 2, 0).cpu().numpy())
        im.save('results_2/{0}/{1}.jpg'.format(self.save_path, txt.replace("/", "")[:200]))
        print(txt)
